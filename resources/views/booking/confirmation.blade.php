            ...
            @if(isset($bookingData))
                <div class="bg-gray-50 rounded-lg p-4 mt-4 text-left space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">{{ $bookingData['name'] ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Email:</span>
                        <span class="font-medium">{{ $bookingData['email'] ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Contact:</span>
                        <span class="font-medium">{{ $bookingData['contact_number'] ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Organization:</span>
                        <span class="font-medium">{{ $bookingData['organization'] ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    @if(isset($bookingData['type']) && $bookingData['type'] == 'walk_in')
                        <div class="flex justify-between border-t pt-2 mt-2">
                            <span class="text-gray-600">Queue Number:</span>
                            <span class="text-xl font-bold text-blue-600">#{{ $bookingData['queue_number'] ?? 'N/A' }}</span>
                        </div>
                    ...
                    @if(isset($event))
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">{{ date('h:i A', strtotime($bookingData['time_slot'])) }}</span>
                        </div>
                        <p class="text-sm text-gray-500 mt-2 text-center">
                    ...
                    <div class="border-t pt-2 mt-2">
                        @if(isset($sms_sent) && $sms_sent)
                            <div class="flex items-center text-green-600 text-sm">
                                ...
                                📱 SMS sent to {{ $bookingData['contact_number'] ?? 'your phone' }}
                            </div>
                        @else
