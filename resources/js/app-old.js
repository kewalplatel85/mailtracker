    ...
    $('#track_number').on('keypress', function(event) {
        ...
        if (event.which === 13) { // Enter key pressed
            ...
            if (custTab === 'Current Clients' && (mailboxError || customerError)) {
                showToast('No match found for mailbox or customer. Please check the inputs.', 'error');
                return;
            }
...
$(document).ready(function () {
    ...
    $('#packageForm').submit(function (event) {
        // Check if this is the old form by looking for specific elements
        if (!$('#custTab1-dropdown-btn').length || !$('.package_stat').length) {
            // This is the new dashboard form, let it handle its own submission
            return;
        }
        ...
        if (!customerName) {
            showToast('Valid customer name and phone number are required.', 'error');
            return;
        }

        if (custTab !== 'New Clients' && (!mailboxNumber || !customerName || !customerPhone)) {
            showToast('Invalid mailbox or customer information. Please check and try again.', 'error');
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Show additional info if SMS was sent
                if (response.sms_sent) {
                    showToast('SMS notification sent to customer', 'info');
                } else if (response.sms_message && response.sms_message !== 'No SMS message provided') {
                    showToast(response.sms_message, 'warning');
                }
